...
CONSIGNEE_CODES = [
    "0000866","0001363","0001540","0001615","0002679","0002990","0003427","0003905",
    "0004932","0005052","0005053","0005056","0005171","0005176","0009633","0013505",
    "0021472","0023453","0028662","0028664","0029594","0030961","0030962","0037361","0048392"
]
...
def write_markdown_report(results):
    with open("eval_results.md", "w") as f:
        ...
        for r in results:
            q_trunc = r['question'][:60] + "..." if len(r['question']) > 60 else r['question']
            f.write(f"| {r['turn']} | {q_trunc} | {r['intent']} | {r['latency_ms']} | {r['tokens']} | {r['cost_usd']:.6f} |\n")
        ...
        for r in results:
            ...
            f.write(f"- **Performance:** {r['latency_ms']}ms | {r['tokens']} tokens | ${r['cost_usd']:.6f}\n\n")
